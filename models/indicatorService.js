...
export async function generateCombinedSignal(pairAddress, currentPrice, currentVolume, currentLiquidity, pairName) {
    ...
    const latestRSI = calculateRSI(priceHistory);
    // const rsiCondition = rsi !== null && rsi <= config.rsiOversold;
    const rsiOk = latestRSI !== null && latestRSI >= 30 && latestRSI <= 60;
    // signalDetails.push(`RSI (${rsi !== null ? rsi.toFixed(2) : 'N/A'}): ${rsiCondition ? '✅ Oversold' : '❌ Not Oversold'}`);
    signalDetails.push(`RSI (${latestRSI !== null ? latestRSI.toFixed(2) : 'N/A'}): ${rsiOk ? '✅ OK (30–60)' : '❌ Out of Range'}`);
    ...
    // --- StochRSI ---
        ...
        const lastStoch = stochRSI.length > 0 ? stochRSI[stochRSI.length - 1] : null;
        const stochCross = lastStoch && lastStoch.k > lastStoch.d;
        const stochOversold = lastStoch && lastStoch.k < 30 && lastStoch.d < 30;
        const stochOk = stochCross || stochOversold;
        if (lastStoch) {
            signalDetails.push(`StochRSI K(${lastStoch.k.toFixed(2)}) D(${lastStoch.d.toFixed(2)}): ${stochCross ? '✅ Bullish Cross' : stochOversold ? '✅ Oversold Bounce' : '❌ No Signal'}`);
        } else {
            signalDetails.push("StochRSI: ❌ Not enough data");
        }
    ...
    const macdCondition = macd && (macd.MACD > macd.signal || macd.histogram > 0 || Math.abs(macd.histogram) < 0.005);
    signalDetails.push(`MACD (${macd?.MACD != null ? macd.MACD.toFixed(4) : 'N/A'} vs Signal ${macd?.signal != null ? macd.signal.toFixed(4) : 'N/A'}): ${macdCondition ? '✅ Bullish or Improving' : '❌ Weak Momentum'}`);
...
//    signalDetails.push(`MACD (${macd?.MACD != null ? macd.MACD.toFixed(4) : 'N/A'} vs Signal ${macd?.signal != null ? macd.signal.toFixed(4) : 'N/A'}): ${macdCondition ? '✅ Bullish Crossover' : '❌ No Bullish Crossover'}`);
    ...
    return {
        ...
        rsi: latestRSI  !== null ? latestRSI.toFixed(2) : 'N/A',
